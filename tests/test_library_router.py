        ...
        with patch(_SC_LOAD, return_value=_valid_tokens()):
            with patch(_INIT_DB):
                with patch(_IMPORT_LIKED, new=AsyncMock(return_value=42)):
                    with patch(_IMPORT_PLAYLISTS, new=AsyncMock(return_value=(5, 120))):
                        resp = client.post("/import")

        assert resp.status_code == 200
        data = resp.json()
        assert data["tracks_added"] == 42
        ...
        with patch(_SC_LOAD, return_value=_valid_tokens()):
            with patch(_INIT_DB):
                with patch(_IMPORT_LIKED, new=AsyncMock(return_value=0)):
                    with patch(_IMPORT_PLAYLISTS, new=AsyncMock(return_value=(0, 0))):
                        resp = client.post("/import")
...
class TestLibraryStats:
    ...
    def test_stats_after_import(self, db_path):
        from app.db import init_db, upsert_playlist, upsert_track
